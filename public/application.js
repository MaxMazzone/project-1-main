...
var onNewMove = function onNewMove(id, userTurn, thereIsWinner, thereIsTie) {
  return $.ajax({
...
var signOutSuccess = function signOutSuccess(gameLogic) {
...
var createNewGameSuccess = function createNewGameSuccess(data) {
  // console.log('new game created')
  // console.log(data)
  store.gameStore = data.game.id;
};

var createNewGameFailure = function createNewGameFailure() {
  // console.log('create new game did not work.')
};
var sayWhosUp = function sayWhosUp(userTurn) {
  $('#game-message').text('Player ' + userTurn + ' is up.');
};
var announceWinner = function announceWinner(userTurn) {
  $('#game-announcement').text(userTurn + ' is the winner!');
  $('#game-announcement').show();
  $('#game-message').hide();
};
var announceTie = function announceTie(thereIsTie) {
  if (thereIsTie === true) {
...
var hideGameMessage = function hideGameMessage() {
  $('#game-announcement').hide();
};
var showInvalidMove = function showInvalidMove() {
...
module.exports = {
  ...
  hideGameMessage: hideGameMessage,
  showInvalidMove: showInvalidMove
...
var thereIsTie = false;
var arrayValueIsNull = function arrayValueIsNull(element, index) {
  return element !== null;
};
...
var resetGame = function resetGame(event) {
  ...
  $('#game-message').show();
  ui.hideGameMessage();
  api.createNewGame().then(ui.createNewGameSuccess).catch(ui.createNewGameFailure);
};
var resetGameOnSignOut = function resetGameOnSignOut(event) {
  event.preventDefault();
  userTurn = 'x';
  thereIsWinner = false;
  thereIsTie = false;
...
var addTokenToArray = function addTokenToArray(userClick, userTurn) {
  gameArray[userClick - 1] = userTurn;
  return gameArray;
};
var putGamePiece = function putGamePiece(id) {
  $('#' + id).text(userTurn);
};
...
var checkForWinner = function checkForWinner(userTurn) {
  var result = false;
  if (checkRow(1, 2, 3, userTurn) || checkRow(4, 5, 6, userTurn) || checkRow(7, 8, 9, userTurn) || checkRow(1, 4, 7, userTurn) || checkRow(2, 5, 8, userTurn) || checkRow(3, 6, 9, userTurn) || checkRow(1, 5, 9, userTurn) || checkRow(3, 5, 7, userTurn)) {
    result = true;
    if (result === true) {
      thereIsWinner = true;
...
var checkRow = function checkRow(a, b, c, userTurn) {
  var result = false;
...
var runGame = function runGame() {
  ...
  } else if (thereIsWinner === true) {} else {}
};
...
var onSignIn = function onSignIn(event) {
  ...
  // console.log(data)
  event.preventDefault();
  api.signIn(data).then(ui.signInSuccess).then(api.createNewGame).then(ui.createNewGameSuccess).catch(ui.signInFailure);
};
...
var addHandlers = function addHandlers() {
  ...
  $('#games-search').on('submit', onGetGames);
};
